...
test('add 3 filters with different priorities and run them', () => {
  ...
  expect(applyFiltersSync('test.filter', 'test')).toBe('testbca')
})
...
test('filters with the same and different priorities', () => {
  ...
  expect(applyFiltersSync('test_order', [])).toEqual([
    '2a',
    '2b',
    '2c',
    '3a',
  ...
  expect(applyFiltersSync('test_order', [])).toEqual(['2a', '2c', '3b', '3c'])
...
test('add and remove an action', () => {
  ...
  expect(doActionSync('test.action')).toBe(undefined)
  expect(window.actionValue).toBe('')
})

test('add an action and run it', () => {
  addAction('test.action', 'my_callback', actionA)
  doActionSync('test.action')
...
test('pass in two arguments to an action', () => {
  ...
  doActionSync('test.action', arg1, arg2)
})
...
test('add a filter before the one currently executing', () => {
  ...
  expect(applyFiltersSync('test.filter', 'test_')).toBe('test_b')
})
...
test('add a filter after the one currently executing', () => {
  ...
  expect(applyFiltersSync('test.filter', 'test_')).toBe('test_ab')
})
...
test('remove specific filter callback', () => {
  ...
  expect(applyFiltersSync('test.filter', 'test')).toBe('testca')
})
...
test('filter removes a callback that has already executed', () => {
  ...
  expect(applyFiltersSync('test.filter', 'test')).toBe('testabc')
})
...
test('filter removes a callback that has not yet executed (middle)', () => {
  ...
  expect(applyFiltersSync('test.filter', 'test')).toBe('testac')
})
...
test('Test doingAction, didAction and hasAction.', () => {
  ...
  doActionSync('unattached.action')
...
test('Verify doingFilter, didFilter and hasFilter.', () => {
  ...
  const test = applyFiltersSync('runtest.filter', 'someValue')
...
test('recursively calling a filter', () => {
  addFilter('test.filter', 'my_callback', (value) => {
    ...
    return applyFiltersSync('test.filter', value + 'X')
  })

  expect(applyFiltersSync('test.filter', 'test')).toBe('testXXX')
})

test('current filter when multiple filters are running', () => {
  addFilter('test.filter1', 'my_callback', (value) => {
    return applyFiltersSync('test.filter2', value.concat(currentFilter()))
  })
  ...
  expect(applyFiltersSync('test.filter1', ['test'])).toEqual([
    'test',
    'test.filter1',
    'test.filter2',
  ])
...
test('adding and removing filters with recursion', () => {
  function removeRecurseAndAdd2(val) {
    ...
    val += '-' + applyFiltersSync('remove_and_add', '') + '-'
  ...
  expect(applyFiltersSync('remove_and_add', '')).toBe('1-134-234')
})
...
test('filters pass first argument across handlers', () => {
  ...
  const result = applyFiltersSync('test.filter', 0)
...
test('adding hooks via composition', () => {
  ...
  expect(typeof testObject.hooks.applyFiltersSync).toBe('function')
})
...
test('adding hooks as a mixin', () => {
  ...
  expect(typeof testObject.applyFiltersSync).toBe('function')
})
...
test('add an all filter and run it any hook to trigger it', () => {
  ...
  expect(applyFiltersSync('test.filter', 'test')).toBe('testa')
  expect(applyFiltersSync('test.filter-anything', 'test')).toBe('testa')
})
...
test('add an all action and run it any hook to trigger it', () => {
  ...
  doActionSync('test.action-anything')
  expect(window.actionValue).toBe('a')
})
...
test('add multiple all filters and run it any hook to trigger them', () => {
  ...
  expect(applyFiltersSync('test.filter', 'test')).toBe('testab')
  expect(applyFiltersSync('test.filter-anything', 'test')).toBe('testab')
})
...
test('add multiple all filters and run it any hook to trigger them by priority', () => {
  ...
  expect(applyFiltersSync('test.filter', 'test')).toBe('testba')
  expect(applyFiltersSync('test.filter-anything', 'test')).toBe('testba')
})
